...
void (*RP2040_SdFile::dateTime_)(uint16_t* date, uint16_t* time) = NULL;

#if ALLOW_DEPRECATED_FUNCTIONS
  // suppress cpplint warnings with NOLINT comment
  void (*RP2040_SdFile::oldDateTime_)(uint16_t& date, uint16_t& time) = NULL;
#endif  // ALLOW_DEPRECATED_FUNCTIONS

//------------------------------------------------------------------------------
// add a cluster to a file
uint8_t RP2040_SdFile::addCluster()
{
...
uint8_t RP2040_SdFile::addDirCluster()
{
  if (!addCluster())
  {
    return false;
  ...
  {
    if (!RP2040_SdVolume::cacheZeroBlock(block + i - 1))
    {
      return false;
    }
  }
...
dir_t* RP2040_SdFile::cacheDirEntry(uint8_t action)
{
  if (!RP2040_SdVolume::cacheRawBlock(dirBlock_, action))
  {
    return NULL;
  }

  return RP2040_SdVolume::cacheBuffer_.dir + dirIndex_;
}
...
*/
uint8_t RP2040_SdFile::createContiguous(RP2040_SdFile* dirFile, const char* fileName, uint32_t size)
{
...
/**
   ...
   the value zero, false, is returned for failure.
*/
uint8_t RP2040_SdFile::dirEntry(dir_t* dir)
{
  ...
  // read entry
  dir_t* p = cacheDirEntry(RP2040_SdVolume::CACHE_FOR_READ);
...
*/
void RP2040_SdFile::dirName(const dir_t& dir, char* name)
{
  uint8_t j = 0;
...
//------------------------------------------------------------------------------
/** List directory contents to DEBUG PORT.
...
*/
void RP2040_SdFile::ls(uint8_t flags, uint8_t indent)
{
  dir_t* p;
...
uint8_t RP2040_SdFile::make83Name(const char* str, uint8_t* name)
{
  uint8_t c;
...
/** Make a new directory.
   ...
   \return The value one, true, is returned for success and
   the value zero, false, is returned for failure.
   Reasons for failure include this RP2040_SdFile is already open, \a dir is not a
...
uint8_t RP2040_SdFile::makeDir(RP2040_SdFile* dir, const char* dirName) 
{
  ...
  // convert RP2040_SdFile to directory
  flags_ = O_READ;
  ...
  dir_t* p = cacheDirEntry(RP2040_SdVolume::CACHE_FOR_WRITE);
  ...
  if (!RP2040_SdVolume::cacheRawBlock(block, RP2040_SdVolume::CACHE_FOR_WRITE)) 
  {
    return false;
  }

  // copy '.' to block
  memcpy(&RP2040_SdVolume::cacheBuffer_.dir[0], &d, sizeof(d));
  ...
  // copy '..' to block
  memcpy(&RP2040_SdVolume::cacheBuffer_.dir[1], &d, sizeof(d));

  // set position after '..'
  curPosition_ = 2 * sizeof(d);

  // write first block
  return RP2040_SdVolume::cacheFlush();
}
//------------------------------------------------------------------------------
/**
   ...
   \return The value one, true, is returned for success and
   the value zero, false, is returned for failure.
   Reasons for failure include this RP2040_SdFile is already open, \a difFile is not
...
uint8_t RP2040_SdFile::open(RP2040_SdFile* dirFile, const char* fileName, uint8_t oflag) 
{
  ...
  while (dirFile->curPosition_ < dirFile->fileSize_) 
  {
    ...
    if (p->name[0] == DIR_NAME_FREE || p->name[0] == DIR_NAME_DELETED) 
    {
      ...
      if (!emptyFound) 
      {
        emptyFound = true;
        dirIndex_ = index;
        dirBlock_ = RP2040_SdVolume::cacheBlockNumber_;
      }
  ...
  if (emptyFound) 
  {
    p = cacheDirEntry(RP2040_SdVolume::CACHE_FOR_WRITE);
  ...
  else 
  {
    ...
    dirIndex_ = 0;
    p = RP2040_SdVolume::cacheBuffer_.dir;
  }
...
uint8_t RP2040_SdFile::openCachedEntry(uint8_t dirIndex, uint8_t oflag) 
{
  // location of entry in cache
  dir_t* p = RP2040_SdVolume::cacheBuffer_.dir + dirIndex;
...
*/
uint8_t RP2040_SdFile::openRoot(RP2040_SdVolume* vol) 
{
...
//------------------------------------------------------------------------------
/** %Print the name field of a directory entry in 8.3 format to DEBUG PORT

   \param[in] dir The directory structure containing the name.
   \param[in] width Blank fill name if length is less than \a width.
*/
void RP2040_SdFile::printDirName(const dir_t& dir, uint8_t width) 
{
  uint8_t w = 0;
...
*/
void RP2040_SdFile::printFatDate(uint16_t fatDate) 
{
  RP2040_SD_LOG1(FAT_YEAR(fatDate), '-');
...
*/
void RP2040_SdFile::printFatTime(uint16_t fatTime) 
{
...
*/
void RP2040_SdFile::printTwoDigits(uint8_t v) 
{
  char str[3];
...
/**
   ...
   or an I/O error occurred.
*/
int16_t RP2040_SdFile::read(void* buf, uint16_t nbyte) 
{
  ...
  while (toRead > 0) 
  {
    ...
    if ((unbufferedRead() || n == 512) && block != RP2040_SdVolume::cacheBlockNumber_) 
    {
    ...
    else 
    {
      ...
      if (!RP2040_SdVolume::cacheRawBlock(block, RP2040_SdVolume::CACHE_FOR_READ)) 
      {
        return -1;
      }
      
      uint8_t* src = RP2040_SdVolume::cacheBuffer_.data + offset;
      uint8_t* end = src + n;
...
*/
int8_t RP2040_SdFile::readDir(dir_t* dir) 
{
  int8_t n;
...
dir_t* RP2040_SdFile::readDirCache() 
{
  ...
  return (RP2040_SdVolume::cacheBuffer_.dir + i);
}
//------------------------------------------------------------------------------
/**
   Remove a file.
...
uint8_t RP2040_SdFile::remove() 
{
  ...
  dir_t* d = cacheDirEntry(RP2040_SdVolume::CACHE_FOR_WRITE);
...
/**
   ...
   or an I/O error occurred.
*/
uint8_t RP2040_SdFile::remove(RP2040_SdFile* dirFile, const char* fileName) 
{
  RP2040_SdFile file;
...
/** Recursively delete a directory and all contained files.
   ...
   the value zero, false, is returned for failure.
*/
uint8_t RP2040_SdFile::rmRfStar() 
{
  rewind();
...
/**
   ...
   the value zero, false, is returned for failure.
*/
uint8_t RP2040_SdFile::seekSet(uint32_t pos)
{
...
*/
uint8_t RP2040_SdFile::sync(uint8_t blocking) 
{
  ...
  return RP2040_SdVolume::cacheFlush(blocking);
}
...
/**
   ...
   the value zero, false, is returned for failure.
*/
uint8_t RP2040_SdFile::timestamp(uint8_t flags, uint16_t year, uint8_t month, uint8_t day, uint8_t hour, uint8_t minute, uint8_t second) 
{
  if (!isOpen() || year < 1980 || year > 2107 || month < 1 || month > 12 || day < 1 || day > 31 || hour > 23 || minute > 59 || second > 59) 
  {
    return false;
  }
...
*/
uint8_t RP2040_SdFile::truncate(uint32_t length) 
{
...
*/
size_t RP2040_SdFile::write(const void* buf, uint16_t nbyte) 
{
  ...
  while (nToWrite > 0) 
  {
    ...
    if (n == 512) 
    {
      ...
      if (RP2040_SdVolume::cacheBlockNumber_ == block) 
      {
        RP2040_SdVolume::cacheBlockNumber_ = 0XFFFFFFFF;
      }
    ...
    else 
    {
      if (blockOffset == 0 && curPosition_ >= fileSize_) 
      {
        ...
        RP2040_SdVolume::cacheBlockNumber_ = block;
        RP2040_SdVolume::cacheSetDirty();
      } 
      else 
      {
      ...
      uint8_t* dst = RP2040_SdVolume::cacheBuffer_.data + blockOffset;
      uint8_t* end = dst + n;
...
/**
   ...
   Use RP2040_SdFile::writeError to check for errors.
*/
size_t RP2040_SdFile::write(uint8_t b) 
{
  return write(&b, 1);
}
//------------------------------------------------------------------------------
/**
   ...
   Use RP2040_SdFile::writeError to check for errors.
*/
size_t RP2040_SdFile::write(const char* str) 
{
...
*/
int RP2040_SdFile::availableForWrite() 
{
  if (!isFile() || !(flags_ & O_WRITE)) 
  {
    return 0;
